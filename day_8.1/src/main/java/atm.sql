...
insert into xr_cash_order(`account_id`, `amount`, `is_success`, `time`, `type`)
values
...
delete from xr_cash_order
where amount > 11;

update xr_cash_order set amount = amount + 1
where order_id=4;

update xr_cash_order set amount = amount + 1, is_success = 0
where order_id=4;


//建表：转账记录
CREATE TABLE `xr_transfer_order`(
   #写数据字段
   `order_id` bigint unsigned auto_increment NOT NULL COMMENT '订单号',
   `to_account_id` bigint unsigned auto_increment NOT NULL COMMENT '收款户id',
   `from_account_id` bigint unsigned auto_increment NOT NULL COMMENT '转款户id',
   `amount` double NOT NULL COMMENT '金额',
   `is_success` tinyint(1) DEFAULT 0 NOT NULL COMMENT '是否成功',
   `time` datetime NULL COMMENT 'datetime',
   primary key (order_id)
...
insert into xr_transfer_order(`to_account_id`,`from_account_id`, `amount`, `is_success`, `time`)
values
(1, 2, 12, 1, '2020-01-01 00:00:00')
,(3, 4, 10, 1, '2022-08-03 00:00:00')
,(3, 4, 40, 1, '2022-08-03 00:00:00')
,(5, 4, 11, 0, '2022-08-03 00:00:00')
;
